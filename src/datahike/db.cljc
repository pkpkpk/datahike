(ns ^:no-doc datahike.db
  (:require
   ...
   [datahike.tools :as tools #?(:clj :refer :cljs :refer-macros) [raise case-tree]]
...
(defrecord-updatable DB [schema eavt aevt avet temporal-eavt temporal-aevt temporal-avet max-eid max-tx op-count rschema hash config system-entities ident-ref-map ref-ident-map meta]
  #?@(:cljs
      [IReversible (-rseq [^DB db] (-rseq eavt))
       IEmptyableCollection (-empty [^DB db] (empty-db (ds/get-user-schema db)))
       IEditableCollection (-as-transient [^DB db] (db-transient db))
       ITransientCollection (-conj! [^DB db key] (throw (ex-info "datahike.DB/conj! is not supported" {})))
       (-persistent! [^DB db] (db-persistent! db))]
...
#?(:cljs
   ...
   (extend-type DB
     ...
     ICounted (-count [^DB db] (count (:eavt db)))
